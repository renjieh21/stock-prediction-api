# 📈 Stock Return Prediction API (PyTorch + FastAPI + Docker)

A complete end‑to‑end machine learning system for **predicting future stock returns** using  
**PyTorch**, **FastAPI**, and **Docker**.  

This project demonstrates full‑stack ML engineering skills, including:

- Data ingestion & feature engineering (yfinance + pandas)
- PyTorch model training & evaluation
- StandardScaler preprocessing export
- Fully deployable FastAPI prediction service
- Dockerized API for portable deployment
- Project structure following MLOps & modern backend best practices

This repository is designed as a strong CS Master’s application project — clear structure, reproducible results, and deployment‑ready API.

> **Note:** The API already includes pre-trained artifacts in the `model/` directory (`model.pth` and `scaler.joblib`).  
> You do **not** need to run `src/train.py` unless you want to retrain or improve the model.  
> The Docker container loads these artifacts automatically at startup.

---

## 🚀 Project Architecture

```
stock-return-prediction-pytorch/
│
├── app/                    # FastAPI application
│   ├── main.py             # Loads model + defines prediction endpoints
│   └── schemas.py          # Pydantic v2 model schemas
│
├── src/                    # ML pipeline (training & data processing)
│   ├── data_loader.py      # Download data & feature engineering
│   ├── dataset.py          # PyTorch Dataset wrapper
│   ├── model_def.py        # MLP regression model
│   └── train.py            # Training script (produces model artifacts)
│
├── model/                  # Saved artifacts (generated by train.py)
│   ├── model.pth           # Trained PyTorch model
│   └── scaler.joblib       # Preprocessing scaler
│
├── Dockerfile              # Containerized API
├── requirements.txt        # Clean Python dependencies
└── README.md               # You are here
```

---

## 📊 1. Data Processing

Data is pulled from Yahoo Finance using `yfinance`, and engineered with:

- 1‑day / 5‑day returns  
- 10‑day / 50‑day SMAs  
- RSI‑14  
- 10‑day rolling volatility  
- Volume change  
- Future *N‑day return* as regression target  

Feature + target creation code lives in:

```
src/data_loader.py
```

Example:

```python
data, feature_cols, target_col = get_processed_data(
    ticker="AAPL",
    start_date="2015-01-01",
    end_date="2023-12-31",
    future_days=5
)
```

---

## 🤖 2. Model Training (PyTorch)

The predictive model is a **Multi‑Layer Perceptron regression network** with:

- BatchNorm  
- Dropout  
- Adam optimizer  
- MSE loss  

Training pipeline: `src/train.py`

```
python src/train.py --ticker "AAPL" --epochs 50
```

Artifacts created:

```
model/model.pth
model/scaler.joblib
```

---

## 🌐 3. FastAPI Deployment

FastAPI app loads the trained model & scaler once at startup:

Endpoints:

| Method | Path             | Description |
|--------|------------------|-------------|
| GET    | `/`              | Health check |
| POST   | `/predict`       | Predict for a single input |
| POST   | `/predict_batch` | Batch inference |

Run locally:

```bash
export PYTHONPATH=$PYTHONPATH:$(pwd)
uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload
```

Interactive docs available at:

➡️ http://127.0.0.1:8000/docs

---

## 🐳 4. Docker Deployment

### Build image:

```bash
docker build -t stock-api .
```

### Run container:

```bash
docker run -p 8000:8000 stock-api
```

API available at:

➡️ http://127.0.0.1:8000/docs

---

## 📦 5. requirements.txt (Clean, Docker‑friendly)

This project uses **Pydantic v2**, which eliminates the metaclass conflicts seen in early runs.

```
fastapi==0.110.0
uvicorn[standard]==0.27.1
torch==2.0.1
scikit-learn==1.3.2
joblib==1.3.2
numpy==1.26.4
pandas==2.1.1
yfinance==0.2.38
pydantic==2.10.3
pydantic-settings==2.2.1
```

---



## 📚 6. Key Skills Demonstrated

This project demonstrates:

- Machine Learning (PyTorch, feature engineering)
- Backend engineering (FastAPI)
- Deployment (Docker)
- Software engineering (modular repo, configs, reproducibility)
- Understanding of financial time‑series

Strong evidence of readiness for CS / AI graduate study.

---

## 📝 Author

Created by **Renjie Hu**,  
Quantitative Risk Analyst & CS/AI enthusiast.

---
